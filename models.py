...
class User(Base):
    ...
    invites_count: Mapped[int] = mapped_column(Integer, default=0)  # necha kishi taklif qilgan
    points: Mapped[int] = mapped_column(Integer, default=0)  # bal tizimi

    is_unlocked: Mapped[bool] = mapped_column(Boolean, default=False)  # guruhga yozish huquqi ochilganmi
...
class Listing(Base):
    ...
    photo_base64: Mapped[str | None] = mapped_column(Text, nullable=True)  # data:image/...;base64,....
    status: Mapped[str] = mapped_column(String(16), default="bosh")  # "bosh" (bo'sh) | "band" (ijaraga berilgan)
...
class BotConfig(Base):
    """Bitta qatorli sozlamalar jadvali (id har doim = 1)."""
